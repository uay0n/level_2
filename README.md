...
## table
### 표 HTML 작성순서와 주의사항
* `table` -> `tr` -> `td` or `th` -> 필요 시 나머지 태그들
* tr 가로 1행 태그, td 내용 1열 태그, th 제목 1열 태그
* 표 태그는 table을 제외하곤 크게 의미로 나누지 않아 행, 열에 클래스 또는 아이디명을 설정하지 않음. 개별 디자인 시에는 css에서 nth 수열선택자를 사용해서 대상을 디자인해야함.
### 표 css 디자인 시 주의사항
* 표는 border가 기본값 0으로 구분이 안되니 css로 구분값 적용하기
* 제목, 내용 열은 height 보다는 padding 상하값으로 상대적 여백 설정하기
* 1행 전체에 동일한 다지안(테두리, 배경색)이 적용되어있을 땐 tr에 적용하기
